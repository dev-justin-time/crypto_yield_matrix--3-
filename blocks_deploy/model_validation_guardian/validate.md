...
| Derived features | `agg_prior`, `agg_current`, `change_pp`, `yield_volatility`, `yield_trend_slope`, `yield_vs_category_avg`, `risk_adjusted_yield`, `yield_momentum`, `mcap_to_tvl`, `risk_score`, `yield_premium` | Recompute and document formulas where possible. `risk_adjusted_yield` is derived from yield and volatility and is not an independent signal. |
