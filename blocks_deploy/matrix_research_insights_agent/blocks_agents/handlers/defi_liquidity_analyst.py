...
def handler(task, ctx=None):
    ...
    for row in rows:
        ...
        findings.append({"symbol": row["symbol"], "volume_current_m_usd": volume, "volume_trend_pct": value(row, "volume_trend_pct"), "tvl_usd": tvl, "active_addresses": value(row, "active_addresses"), "daily_transactions": value(row, "daily_tx_count"), "market_cap_usd": value(row, "mcap_end_current_usd"), "volume_to_tvl": volume * 1_000_000 / tvl if volume is not None and tvl not in (None, 0) else None, "mcap_to_tvl": features["mcap_to_tvl"], "asset_enrichment": asset_enrichment(row["symbol"]), "market_snapshot_research": snapshot_research(row["symbol"]), "evidence": evidence(row, filename)})
