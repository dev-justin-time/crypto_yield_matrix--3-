...
def handler(task, ctx=None):
    ...
    return report("quant_forecasting_expert", "FAIL", "Forecast execution is blocked for production use until source identity is resolved and additional dated observations are added.", [{"check": "source_selected", "pass": payload.get("source_file") in {"yield_data.csv"}}, {"check": "additional_history", "pass": False, "reason": "Current project has eight quarterly observations per asset."}, {"check": "walk_forward_validation", "pass": False, "reason": "No fitted model or independent future outcomes were supplied."}], assumptions=["A request may use the canonical yield_data.csv dataset for prototype analysis."], limitations=["The current eight-quarter panel and embedded provenance rows are unsuitable for validated production forecasting.", "No point forecast is returned."], source_file=source, context_files=list(dict.fromkeys(["validate.md", "DATA_DICTIONARY.md"] + payload.get("files", []))))
